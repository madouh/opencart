...
class ControllerStep2 extends Controller {
	...
	private function validate() {
		if (phpversion() < '5.3') {
			$this->error['warning'] = '! تحذير : أنت تحتاج الى استخدام اصدار بي اتش بي الخامس أو أعلى لكي يعمل متجر أوبن كارت';
		}

		if (!ini_get('file_uploads')) {
			$this->error['warning'] = '! file_uploads تحذير : يجب تفعيل خاصية رفع الملفات في السيرفر';
		}

		if (ini_get('session.auto_start')) {
			$this->error['warning'] = '! session.auto_start تحذير : متجر اوبن كارت لن يعمل في حالة تفعيل خاصية بدء الجلسة التلقائي';
		}

		if (!array_filter(array('mysql', 'mysqli', 'pdo', 'pgsql'), 'extension_loaded')) {
			$this->error['warning'] = '!  تحذير : يجب تشغيل اضافة قواعد البيانات mysqli في ملف php.ini لكي يعمل متجر أوبن كارت';
		}

		if (!extension_loaded('gd')) {
			$this->error['warning'] = '! GD تحذير : لكي يعمل متجر أوبن كارت يجب تشغيل اضافة';
		}

		if (!extension_loaded('curl')) {
			$this->error['warning'] = '! CURL تحذير : لكي يعمل متجر أوبن كارت يجب تشغيل اضافة';
		}

		if (!function_exists('mcrypt_encrypt')) {
			$this->error['warning'] = '! mCrypt تحذير : لكي يعمل متجر أوبن كارت يجب تشغيل اضافة';
		}

		if (!extension_loaded('zlib')) {
			$this->error['warning'] = '! ZLIB تحذير : لكي يعمل متجر أوبن كارت يجب تشغيل اضافة';
		}

		if (!extension_loaded('zip')) {
			$this->error['warning'] = '! ZIP تحذير : لكي يعمل متجر أوبن كارت يجب تشغيل اضافة';
		}

		if (!function_exists('iconv')) {
			if (!extension_loaded('mbstring')) {
				$this->error['warning'] = '! mbstring تحذير : لكي يعمل متجر أوبن كارت يجب تشغيل اضافة';
			}
		}

		if (!file_exists(DIR_OPENCART . 'config.php')) {
			$this->error['warning'] = 'يجب تغيير اسم الملف config.php إلى config-dist.php';
		} elseif (!is_writable(DIR_OPENCART . 'config.php')) {
			$this->error['warning'] = 'تحذير : يجب اعطاء التصريح المطلوب لملف الكونفيج config.php';
		}

		if (!file_exists(DIR_OPENCART . 'admin/config.php')) {
			$this->error['warning'] = 'يجب تغيير اسم الملف admin/config-dist.php إلى admin/config.php';
		} elseif (!is_writable(DIR_OPENCART . 'admin/config.php')) {
			$this->error['warning'] = 'تحذير : يجب اعطاء التصريح المطلوب لملف الكونفيج admin/config.php';
		}

		if (!is_writable(DIR_OPENCART . 'image')) {
			$this->error['warning'] = '! image تحذير : يجب أن يكون مجلد الصور قابل للكتابة لكي يعمل متجر اوبن كارت';
		}

		if (!is_writable(DIR_OPENCART . 'image/cache')) {
			$this->error['warning'] = '! image/cache تحذير : يجب أن يكون مجلد ملفات كاش الصور قابل للكتابة لكي يعمل متجر اوبن كارت';
		}

		if (!is_writable(DIR_OPENCART . 'image/catalog')) {
			$this->error['warning'] = '! image/catalog تحذير : يجب أن يكون مجلد ملفات الصور قابل للكتابة لكي يعمل متجر اوبن كارت';
		}
